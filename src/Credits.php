...
use MoneyMath\DecimalWholeNumbers;
use Nette\Object;
 ...
 */
class Credits extends Object {

    /**
     * Internal value
     *
     * @var DecimalWholeNumbers
     */
    private $amount;
     ...
     *
     * @param int|DecimalWholeNumbers $amount
     *
     * @return Money
     */
     ...
     * @return boolean
     */
    public function equals(Credits $other) {
     ...
     * @return int
     */
    public function compare(Credits $other) {
     ...
     * @return boolean
     */
    public function greaterThan(Credits $other) {
     ...
     * @return boolean
     */
    public function lessThan(Credits $other) {
        return -1 == $this->compare($other);
    }
     ...
     * @return Money
     */
    public function add(Credits $addend) {
     ...
     * @return Money
     */
    public function subtract(Credits $subtrahend) {
    ...
    public function allocate(array $ratios) {
        ...
        foreach ($ratios as $ratio) {
            $share = $this->amount->multiplyBy($ratio)->divide(DecimalWholeNumbers::from($total));
            $results[] = new Credits($share);
            $remainder = $remainder->subtract($share);
        }
        for ($i = 0; $remainder->isPositive(); $i++) {
            $results[$i] = Credits::from($results[$i]->amount->add(1));
    ...
    public static function stringToUnits($string) {
        ...
        $decimals = "(?P<decimal1>\d)?(?P<DecimalWholeNumbers>\d)?";
        ...
        $units .= isset($matches['DecimalWholeNumbers']) ? $matches['DecimalWholeNumbers'] : "0";
